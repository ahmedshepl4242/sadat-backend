...
async function verifyOrderStatus(orderId, expectedStatus) {
  try {
    logger.info(`Verifying order status for order ${orderId}: expected ${expectedStatus}`);
...
async function adminSearchUser() {
  ...
  try {
    ...
    const userFound = response.data.users.some(user => user.id === STATE.ids.user);
...
async function vendorSearchByAdmin() {
  ...
  try {
    ...
    const vendorFound = response.data.vendors.some(vendor => vendor.id === STATE.ids.vendor);

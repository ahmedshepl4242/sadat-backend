...
async function loginSystemVendor() {
  ...
  logger.success(
    "✅ Skipping system vendor login (using admin token directly)",
  );
...
async function systemVendorCounterOffer() {
  ...
  logger.info("Current tokens state", {
    ...
    adminToken: STATE.tokens.admin
      ? STATE.tokens.admin.substring(0, 20) + "..."
      : "undefined",
    systemVendorToken: STATE.tokens.systemVendor
      ? STATE.tokens.systemVendor.substring(0, 20) + "..."
      : "undefined",
  });

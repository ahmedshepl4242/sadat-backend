...
// Test files to run
const testFiles = [
  { name: 'Happy Order Flow', file: 'test_happy_order_flow.js', description: 'Standard user-initiated order flow' },
  { name: 'Vendor Initiated Order Flow', file: 'test_vendor_initiated_order_flow.js', description: 'Vendor creates order for external customer' },
  { name: 'Special Order Flow', file: 'test_special_order_flow.js', description: 'Order with system vendor' }
];
...
async function runTest(testInfo) {
  ...
  try {
    ...
    console.log(`✅ ${testInfo.name} completed successfully in ${duration.toFixed(2)} seconds`);
    
    // Try to extract timing stats from the log file
    try {
      const logContent = await fs.readFile(testInfo.file.replace('.js', '_results.log').replace('test_special_order_flow_results.log', 'test_special_order_results.log'), 'utf8');
      ...
      console.log(`⚠️  Could not read timing stats from log for ${testInfo.name}`);
      timingResults.push({
        testName: testInfo.name,
        success: true,
        duration
    ...
    console.log(`❌ ${testInfo.name} failed after ${duration.toFixed(2)} seconds`);
...
async function runAllTests() {
  ...
  for (const result of timingResults) {
    ...
    if (result.timingStats) {
      console.log(`   Operations: ${result.timingStats.totalOperations}, Avg: ${result.timingStats.averageDuration.toFixed(2)}ms`);
    }
  }
  ...
  console.log(`Average Test Duration: ${(totalDuration / testFiles.length).toFixed(2)}s`);

...
} = require("./test_vendor_initiated_order_flow");
const { runSpecialOrderFlow } = require("./test_special_order_flow");

async function runComprehensiveTests() {
  console.log(
    "🚀 Starting Comprehensive Delivery System Tests with Timing Statistics",
  );
  console.log(
    "=====================================================================",
  );
  ...
  for (const result of testResults) {
    ...
    if (result.success) {
      ...
      if (result.timingStats) {
        ...
        console.log(
          `   Avg Operation Time: ${stats.averageDuration.toFixed(2)}ms`,
        );
  ...
  console.log(
    `Success Rate: ${((passedTests / testResults.length) * 100).toFixed(1)}%`,
  );
  console.log(`Total Execution Time: ${overallDuration.toFixed(2)}s`);
  console.log(`Total API Requests: ${totalRequests}`);
  console.log(
    `Average Test Duration: ${(totalTestDuration / passedTests || 0).toFixed(2)}s`,
  );
